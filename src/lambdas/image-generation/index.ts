...
function extractVisualDescription(sceneContent: string): string {
  ...
  let description = sceneContent.replace(/"[^"]*"/g, "");

  // Extract visual cues in brackets or parentheses
  const visualCues: string[] = [];
  const bracketMatches = description.match(/\[[^\]]+\]/g);
  const parenMatches = description.match(/\([^)]+\)/g);

  if (bracketMatches) {
    visualCues.push(...bracketMatches.map((m) => m.slice(1, -1)));
  }
  if (parenMatches) {
    visualCues.push(...parenMatches.map((m) => m.slice(1, -1)));
  }
  ...
  // Combine main description with visual cues
  let finalDescription = description;
  if (visualCues.length > 0) {
    finalDescription += ". " + visualCues.join(". ");
  }

  // Limit description length for Bedrock
  if (finalDescription.length > 500) {
    finalDescription = finalDescription.substring(0, 500) + "...";
  }

  // Add manga style context
  finalDescription = `Manga style illustration: ${finalDescription}. Black and white manga art style with detailed line work and dramatic shading.`;
